...
final class WeekdayProviderTest: XCTestCase {
	static var container: PersistentContainer = PersistentContainer(with: .testing)
	...
	func test_CreateWeekdaysInitially() throws {
		...
		let weekdayProvider = PersistentTodoListProvider(persistentContainer: Self.container)
		
		XCTAssertEqual(weekdayProvider.sections.count, 7)
		
		XCTAssertEqual(weekdayProvider.sections[0].date.testFormat, dayTitle(in: 0))
		XCTAssertEqual(weekdayProvider.sections[0].todos.count, 3)
		...
		XCTAssertEqual(weekdayProvider.sections[1].todos.count, 4)
		...
		XCTAssertEqual(weekdayProvider.sections[2].todos.count, 3)
		...
		XCTAssertEqual(weekdayProvider.sections[3].todos.count, 3)
		...
		XCTAssertEqual(weekdayProvider.sections[4].todos.count, 3)
		...
		XCTAssertEqual(weekdayProvider.sections[5].todos.count, 3)
		...
		XCTAssertEqual(weekdayProvider.sections[6].todos.count, 1)
	...
	func test_CreateWeekdaysExcluded() throws {
		ExcludedDates.shared.removeAll()
		ExcludedDates.shared.manuallyExclude(date: .now)
		ExcludedDates.shared.manuallyExclude(date: .now.addingTimeInterval(86400 * 3))
		
		let weekdayProvider = PersistentTodoListProvider(persistentContainer: Self.container)

		XCTAssertEqual(weekdayProvider.sections.count, 8)
		
		XCTAssertEqual(weekdayProvider.sections[0].isManuallyExcluded, true)
		XCTAssertEqual(weekdayProvider.sections[1].date.testFormat, dayTitle(in: 1))
		XCTAssertEqual(weekdayProvider.sections[1].todos.count, 7)
		...
		XCTAssertEqual(weekdayProvider.sections[2].todos[1].title, "Einkäufe für das Wochenende erledigen")
		XCTAssertEqual(weekdayProvider.sections[2].todos[2].title, "Arzttermin für Impfung vereinbaren")
		XCTAssertEqual(weekdayProvider.sections[3].isManuallyExcluded, true)
		XCTAssertEqual(weekdayProvider.sections[4].date.testFormat, dayTitle(in: 4))
		XCTAssertEqual(weekdayProvider.sections[4].todos.count, 3)
		...
		XCTAssertEqual(weekdayProvider.sections[6].date.testFormat, dayTitle(in: 6))
		XCTAssertEqual(weekdayProvider.sections[6].todos.count, 3)
		...
		XCTAssertEqual(weekdayProvider.sections[7].todos.count, 1)
